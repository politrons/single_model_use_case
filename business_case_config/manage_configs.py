...
from contract_utilities.generate_contract import (
    ...
    update_bundle_include,
    generate_factory_and_batch_resources,
)
...
databricks_bundle_path = Path("../databricks.yml")
...
#Resources
if resources_items_to_delete:
    print(f"Deleting all {len(resources_items_to_delete)} items in resources/...\n")
    for item in resources_items_to_delete:
        print(f"Deleting resource item {item.name} ...")
        if item.is_dir():
            shutil.rmtree(item, ignore_errors=True)
        else:
            item.unlink(missing_ok=True)
    print("All resources content deleted.\n")
else:
    print("No existing resources content found to delete.\n")
...
# COMMAND ----------
# Create resources jobs

all_models_created = sorted(set(ibnr_models_created + inflation_models_created))
new_resource_files = []
for model_name in all_models_created:
    new_resource_files.append(resources_path / f"factory-{model_name}-resource.yml")
    new_resource_files.append(resources_path / f"batch-{model_name}-resource.yml")
updated_include = update_bundle_include(
    bundle_file_path=databricks_bundle_path,
    resource_paths=new_resource_files,
)
print(f"Updated databricks.yml include with {len(updated_include)} resources")
