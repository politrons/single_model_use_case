...
def get_aggregated_table_name(
    ...
    prefix = 'ibnr_aggregated'
    if len(segmentation) == 0:
        segmentation_str = '__none'
    else:
        segmentation_str = '__' + '_'.join(segmentation)

    full_name = prefix + f'__preauth_{preauth_date_str}' + segmentation_str
...
def get_run_config(
    ...
    all_timeframes = get_inference_timeframe_ranges('ibnr', relative_path)
                    ...
                    for target in inner_config['targets']:
                        for feature_combination in inner_config['input_features_combinations']:
                            for model_config in inner_config['model_configs']:
                                ...
                                job_compute = 'serverless'
                                if ('MajorICDGroupingDescription' in segmentation_columns) and is_per_lag_modeling:
                                ...
                                FullExperiment = f'{Experiment_without_target}_{target}'
...
def _get_model_experiment_name(model_type: str, model_config: dict) -> str:
    ...
    if model_type == 'neural_network':
        ...
        s += ' | ChainLadderConfig' ############## TODO: implement this
    else:
...
def merge_duplicate_models(models_created: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for name, group in groups.items():
        ...
        if len(group) != 2:
            ...
            raise ValueError(
                f"Model name '{name}' appears {len(group)} times. "
                "Only exactly 1 or 2 occurrences are supported."
            )
        ...
        for item in group:
            ...
            if s is True and i is False:
                if standalone_item is not None:
                    raise ValueError(
                        f"Model name '{name}' has multiple items with "
                        "ibnr_standalone=True / ibnr_inflation=False"
                    )
                ...
                if inflation_item is not None:
                    raise ValueError(
                        f"Model name '{name}' has multiple items with "
                        "ibnr_standalone=False / ibnr_inflation=True"
                    )
                inflation_item = item
            else:
                raise ValueError(
        ...
        if standalone_item is None or inflation_item is None:
            raise ValueError(
                f"Model name '{name}' does not contain exactly one item of each "
                "required flag pattern (standalone=True/inflation=False and the inverse)."
            )

        # Verify all other keys have identical values
        keys_to_check = [
            k for k in standalone_item.keys()
            if k not in ("ibnr_standalone", "ibnr_inflation")
        ]
        for key in keys_to_check:
            if standalone_item[key] != inflation_item[key]:
                raise ValueError(
                    f"Model name '{name}' has differing values for key '{key}' "
                    "between the two duplicate entries."
                )
    ...
    assert len(all_names) == len(set(all_names)), (
        "Model names are not unique after duplicate merging. "
