...
_DEFAULT_OCCURRENCE_DATE_COL = "TreatmentDate"
...
class ChainLadderModel(BaseEstimator, RegressorMixin):
    ...
    def __init__(
        ...
        self.occurrence_date_col: str = self.config.get(
            CONFIG_OCCURRENCE_DATE_COL, _DEFAULT_OCCURRENCE_DATE_COL
        )
    ...
    def fit(
        ...
        **extra_params
        ...
        cumulative_cols = [
            ...
            if self.cumulative_identifier in c
        ]
        if len(cumulative_cols) != 1:
            raise ValueError(
                f"Expected exactly one column containing "
                f"'{self.cumulative_identifier}' in its name, "
                f"found {len(cumulative_cols)}: {cumulative_cols}"
            )
        ...
        df_train["Next_Cumulative"] = (
            df_train
            .groupby(group_keys)[self.cumulative_col_]
            .shift(-1)
        )
        ...
        df_train.loc[mask, "Development Factor"] = (
            df_train.loc[mask, "Next_Cumulative"]
            / df_train.loc[mask, self.cumulative_col_]
        )
        ...
        df_factors = (
            df_train
            .groupby(factor_group_keys)["Development Factor"]
            .mean()
            .reset_index()
        )
        df_factors.rename(
        ...
        df_factors["Avg Development Factor"] = (
            df_factors["Avg Development Factor"].fillna(1.0)
        )
        ...
        if not self.segmentation_cols_:
            df_factors["Cumulative Factor"] = (
                df_factors["Avg Development Factor"].cumprod()
            )
        else:
            df_factors["Cumulative Factor"] = (
                df_factors
                .groupby(self.segmentation_cols_)["Avg Development Factor"]
                .cumprod()
            )
        ...
        logger.info(
            f"Chain ladder fitted — "
            f"{len(df_factors)} factor rows, "
            f"{self.lag_col} range "
            f"[{df_factors[self.lag_col].min()}, {df_factors[self.lag_col].max()}]"
        )
    ...
    def predict(
        ...
        predictions = (
            df[self.cumulative_col_] * df["Cumulative Factor"]
